    ...
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {particles.map((p) => (
        <motion.div
          ...
          animate={{ y: [0, -30, 0], x: [0, 20, -20, 0], opacity: [0, 1, 0] }}
...
const StatusBadge = ({ icon: Icon, label, color }: { icon: React.ElementType; label: string; color: StatusColor }) => {
  const colors = STATUS_BADGE_COLORS[color];
  return (
    <div className={cn("flex items-center gap-2 px-3 py-1 rounded-full border", colors.bg, colors.border)}>
...
const STAT_COLORS = {
  "from-blue-500 to-cyan-500":    { bg: "from-blue-500 to-cyan-500",    glow: "bg-blue-500/20"   },
  "from-purple-500 to-pink-500":  { bg: "from-purple-500 to-pink-500",  glow: "bg-purple-500/20" },
  "from-orange-500 to-red-500":   { bg: "from-orange-500 to-red-500",   glow: "bg-orange-500/20" },
  "from-green-500 to-emerald-500":{ bg: "from-green-500 to-emerald-500",glow: "bg-green-500/20"  },
} as const;

type StatColor = keyof typeof STAT_COLORS;

const StatItem = ({ icon: Icon, label, value, color, delay }: {
  icon: React.ElementType; label: string; value: number; color: StatColor; delay: number;
}) => {
  ...
  return (
    ...
    >
      <div className={cn(
        "relative p-4 rounded-xl backdrop-blur-sm border transition-all duration-300 bg-white/5 dark:bg-black/20",
        isHovered ? "border-cyan-400/50 shadow-[0_0_30px_rgba(6,182,212,0.2)]" : "border-white/10",
      )}>
        <div className={cn("absolute -inset-1 rounded-xl opacity-0 transition-opacity duration-300 blur-xl", isHovered && "opacity-100", colors.glow)} />
        <div className="relative z-10 flex items-center gap-3">
          <div className={cn("w-10 h-10 rounded-lg flex items-center justify-center transition-all duration-300 bg-gradient-to-br", colors.bg, isHovered && "scale-110 shadow-lg shadow-cyan-500/25")}>
        ...
        <div className={cn("absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-cyan-400 to-transparent transition-all duration-1000", isHovered ? "opacity-100" : "opacity-0")} />
      </div>
    </motion.div>
  );
};
...
export function DashboardHero() {
  ...
  const [systemStats, setSystemStats] = useState({ uptime: 0, node: 0, cpu: 0, mem: 0 });
  ...
  // Lấy tên riêng (từ cuối) của người dùng, an toàn với mọi trường hợp
  const displayName = (() => {
    if (!isMounted || !session?.user?.name) return "Bạn";
    const parts = session.user.name.trim().split(/\s+/).filter(Boolean);
    return parts.length > 0 ? parts[parts.length - 1] : session.user.name;
  })();
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-2 text-white/40 text-xs font-mono">
          ...
          <span>{isMounted ? new Date().toLocaleTimeString("vi-VN") : "--:--:--"}</span>
      ...
      >
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          ...
          <div className="flex-1">
            ...
            >
              ...
              <span className="text-white/60 text-sm font-medium tracking-wider">Dashboard</span>
            ...
            >
              ...
              <motion.span
                ...
                className="text-cyan-400 font-semibold text-lg md:text-2xl mt-1 block"
              >
            ...
            >
              Hệ thống quản lý học tập thông minh • Công nghệ AI • Dữ liệu thời gian thực
            </motion.p>
          ...
          >
            ...
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 backdrop-blur-sm">
              ...
              <span className="text-xs text-white/80 font-mono">Cloud Sync</span>
            </div>
        ...
        >
          <StatItem icon={Users}      label="Thành viên" value={stats.students || 0}                                    color="from-blue-500 to-cyan-500"     delay={0.8} />
          <StatItem icon={BookOpen}   label="Tài liệu"   value={stats.documents || 0}                                   color="from-purple-500 to-pink-500"   delay={0.9} />
          <StatItem icon={Video}      label="Bài giảng"  value={stats.lectures || 0}                                    color="from-orange-500 to-red-500"    delay={1.0} />
          <StatItem icon={TrendingUp} label="Tăng trưởng"value={stats.documents ? Math.floor(stats.documents * 1.2) : 0} color="from-green-500 to-emerald-500" delay={1.1} />
        </motion.div>
      </motion.div>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <span className="hidden sm:inline">•</span>
          <span className="hidden sm:inline">UPTIME: {isMounted ? systemStats.uptime : 0}h</span>
          <span className="hidden lg:inline">•</span>
          <span className="hidden lg:inline">NODE: {isMounted ? systemStats.node : 0}/12</span>
        </div>
        <div className="flex items-center gap-4">
