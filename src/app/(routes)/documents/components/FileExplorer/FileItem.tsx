...
export function FileItem({
  ...
  const handleDoubleClick = () => {
    if (isFolder) {
      ...
      // ✅ Increment view counter like forum posts
      fetch("/api/documents/increment", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ document_id: item.id, type: "view" }),
  ...
  return (
    ...
    >
      ...
      {/* Stats: Views, Downloads, Rating - Chỉ hiển thị cho file */}
      {!isFolder && !isRenaming && item.views !== undefined && (
        <div className="flex items-center justify-center gap-2 mt-1 text-[10px] text-white/30">
          ...
          <span className="flex items-center gap-0.5" title="Đánh giá">
            <Star
              className={`w-3 h-3 ${(item.rating_avg || 0) > 0 ? "fill-yellow-400 text-yellow-400" : ""}`}
            />
